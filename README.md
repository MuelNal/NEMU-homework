...
思考题3：宏在预处理阶段已经被文本替换了，编译器识别不了宏的存在所以无法写入符号表，局部变量c和形参a,b都是局部变量，当程序运行的时候分配在栈上，通过栈指针来访问，因此不算符号，符号具有编译时期可以确定的地址或者相对偏移量，通常包括全局变量，函数，静态变量或者静态函数
思考题4：栈用于函数调用、保存返回地址、局部变量、它的大小在编译的时候完全无法预知，堆用于动态分配内存，因此栈和堆都是运行时状态，操作系统在加载程序的时候通过虚拟内存管理建立虚拟地址空间，在高地址处分配一块区域作为栈，并将栈指针寄存器指向这块区域；在数据段分配空间作为堆，当程序申请内存的时候，操作系统再分配物理内存并扩展堆
思考题5：通过文件的魔数进行识别，elf格式与PE格式文件头几个字节是不一样的，当系统调用时，会读取文件的前几个字节，如果加载器不认识这个魔数，就会返回错误
思考题6：已经初始化的变量放在.data段，未初始化的变量或者初始化为0的变量放在.bss段，因此文件里面只需要存.data的部分，也就是filesize部分，但是内存中需要分配.data和.bss的总大小，也就是memsize部分，因为memsize还要包括.bss的部分，所以因此memsize一般比filesize的大小更大

main 返回后会回到 C 运行时库的 __libc_start_main 函数，最终由 exit() 结束进程。